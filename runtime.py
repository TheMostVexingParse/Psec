...
class Interpreter:
    ...
    def fooish_eval(self, text):
        for proh in ["not", "in", "is", "and", "or"]:
            if f" self.{proh} " in text:
                text = text.replace(f" self.{proh} ", f" {proh} ")
        try:
            return eval(text)
        except NameError as ne:
            var_name = re.findall(r"'([^']*)'", str(ne))[0]
            return (eval(text.replace(var_name, f"self.{var_name}")), )
...
def execute_backend(code, debug=False):
    ...
    while break_condition:
        try:
            ...
            if line_exec == []:
                line += 1
                continue  # en alttaki line arttırma satırı kaldırılabilir
            if line_exec[0] == "//":
                line += 1
                continue  # ama çıkabilecek hatalarla uğraşmaya üşeniyorum
            if exec_else:
            ...
            if line_exec[0].lower() == "eğer":
                ...
                for var_name_found in var_names(to_eval):
                    to_eval = to_eval.replace(f" {var_name_found} ", f" self.{var_name_found} ")
            ...
            if line_exec[0].lower() == "bitir":
                ...
                try:
                    session.fooish_eval(f"self.{listname}.append({object})")
                except Exception:
                    try:
                        session.fooish_eval(f"self.{listname}.append(self.{object})")
                    except Exception:
                ...
                try:
                    line = int(line_exec[1].lower().strip("a")) - 2
                except ValueError:
                    try:
                        for i in range(len(code)):
                            if code[i].strip() == f":{line_exec[1]}":
                                line = i
                                break
                        continue
                    except Exception:
                        custom_output(
                            f"Error: Can't find mentioned step -> {line_exec[1]}",
                            debug=debug,
                        )
                    break
        except IndexError:
        ...
        line += 1
    if break_condition:
        custom_output("Terminating process...", False, debug=debug)
    else:
...
def execute(code, debug=False):
    ...
    try:
        execute_backend(to_exec, debug=debug)
